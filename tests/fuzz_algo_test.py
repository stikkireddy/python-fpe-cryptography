...
crypto_encrypt_method = functools.partial(fpe_encrypt_or_decrypt,
                                          ff3_cipher_klass=CryptoFF3Cipher,
                                          operation="ENCRYPT")
pycryptodome_encrypt_method = functools.partial(fpe_encrypt_or_decrypt,
                                        ff3_cipher_klass=PyCryptodomeFF3Cipher,
                                        operation="ENCRYPT")
crypto_decrypt_method = functools.partial(fpe_encrypt_or_decrypt,
                                            ff3_cipher_klass=CryptoFF3Cipher,
                                            operation="DECRYPT")
pycryptodome_decrypt_method = functools.partial(fpe_encrypt_or_decrypt,
                                                ff3_cipher_klass=PyCryptodomeFF3Cipher,
                                                operation="DECRYPT")
...
def single_fuzz_test(iteration):
    ...
    pycryptodome_ciphertext = pycryptodome_encrypt_method(text=plaintext, key=key, tweak=tweak)
    crypto_ciphertext = crypto_encrypt_method(text=plaintext, key=key, tweak=tweak)

    assert crypto_ciphertext == pycryptodome_ciphertext, f"Iteration {iteration}: Encryption mismatch"

    crypto_decrypted = crypto_decrypt_method(text=crypto_ciphertext, key=key, tweak=tweak)
    pycryptodome_decrypted = pycryptodome_decrypt_method(text=pycryptodome_ciphertext, key=key, tweak=tweak)
